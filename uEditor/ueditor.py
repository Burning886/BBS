...
@ueditor.before_app_first_request
...
@ueditor.route('/upload/', methods=['GET', 'POST'])
def upload():
    action = request.args.get('action')
    result = {}
    if action == 'config':
        config_path = os.path.join(ueditor.static_folder or app.static_folder, 'ueditor', 'config.json')
